...
start_time <- Sys.time()
gc()  # garbage collection to clean memory
...
gc_output <- gc()
memory_usage <- sum(gc_output[,2]) # Used memory in MB

cat("\nExecution Time:", as.numeric(execution_time), "seconds\n")

cat("Memory Used (approx):", memory_usage, "MB\n")
